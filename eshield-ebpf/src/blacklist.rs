...
pub fn is_blacklisted(src_ip: u32, now_ns: u64) -> bool {
    match unsafe { BLACKLIST.get(&src_ip) } {
        Some(entry) => {
    ...
    // 已过期，从黑名单中移除
    let _ = BLACKLIST.remove(&src_ip);
    false
}

pub fn emit_blacklist_event(_ctx: &XdpContext, src_ip: u32, protocol: u8) {
    unsafe {
