...
CFG=/tmp/eshield-phases.toml
LOG=/tmp/eshield-phases.log
REDB=/tmp/eshield-phases.redb
rm -f "$CFG" "$LOG" "$REDB"
cat > "$CFG" <<EOF
...
api_stats() {
    ip netns exec "$SERVER_NS" curl -sS --connect-timeout 2 "http://127.0.0.1:8443/api/stats" | jq -c '{total_dropped,udp_flood_blocked,icmp_flood_blocked,syn_flood_blocked,blacklist_blocked}' || true
}

echo "baseline: $(api_stats)"

echo "--- UDP flood 2s ---"
timeout 2s ip netns exec "$CLIENT_NS" hping3 --flood --udp -p 53 "$SERVER_IP" >/dev/null 2>&1 || true
sleep 1
echo "after UDP: $(api_stats)"
sleep 2
echo "after 2s: $(api_stats)"
sleep 2
echo "after 4s: $(api_stats)"
