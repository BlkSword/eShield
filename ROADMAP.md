...
**目标**：实现多节点策略共享，一个节点发现威胁，所有节点共享免疫记忆。

- [x] `eshield-hub` 独立二进制（axum + redb）
- [x] 节点 → Hub 策略上报（黑名单、Trust Score）
- [x] Hub → 节点策略分发（peer 策略、Hub 威胁情报）
- [x] tombstone 删除同步
- [x] 规则包统一下发（ACL / L7 / 防护项目）
- [x] Hub Dashboard + 节点集群页面
- [x] 多 Hub URL 故障转移
...
**目标**：提升应用层防御能力，引入规则有效性追踪与自动演化。

### 1. 控制台视觉升级

- [x] 参考雷池 WAF 重新设计 Dashboard 视觉风格（更清爽的配色、更专业的图标、更合理的留白）
- [x] 替换 emoji 为统一 SVG 图标
- [x] 优化字体、字号、行距与视觉层级
- [x] 统一卡片、表格、按钮、表单样式
- [x] 增加页面切换动画与微交互
...
- [x] 告警 Webhook（Slack / 钉钉 / 企业微信 / 自定义 HTTP）
...
- [x] 单元测试覆盖核心模块（`parser`、`syn_cookie`、`rate_limit`、`config`）
- [ ] 属性测试：随机生成畸形包、非法 CIDR、边界阈值
- [x] 负向集成测试：放行流量可达、白名单不误杀
- [x] IPv6 / UDP / ICMP 集成测试扩展
...
- [ ] **多集群联邦**
  - 跨 Region/机房的多 Hub 联邦
  - 基于 gossip 在 Hub 间同步高置信策略
