...
- 防护项目：按协议 + 端口 + 目标 IP 分组配置策略（v0.4.6 起 PASS/DROP 在数据面生效，DEFEND 复用全局防御模块）。
...
> 仅支持 IPv4 TCP。**降级式挑战**：正常连接始终直通内核（零影响）；源 IP 触发 SYN Flood 阈值后进入 Cookie 挑战模式——SYN 被改写为 SYN-ACK Cookie 挑战包，伪造源无法通过验证、在 XDP 层被清洗；合法客户端响应 Cookie 后自动解除挑战，重试连接即恢复正常。
